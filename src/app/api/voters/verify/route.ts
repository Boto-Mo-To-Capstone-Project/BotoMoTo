...
export async function POST(request: NextRequest) {
  try {
    ...
    if (positions.some(pos => pos.candidates.some(c => c.credentialObjectKey))) {
      console.log("Credentials object keys found for candidates in ballot data.");
      console.log(positions.flatMap(pos => pos.candidates.map(c => c.credentialObjectKey)));
    } else {
      console.log("No credentials object keys found for candidates in ballot data.");
    }
